...
function genCode(){let s='';for(let i=0;i<19;i++)s+=Math.floor(Math.random()*10);const dv=(10-[...s].reduce((a,c,i)=>a+(+c)*(i%2?1:2),0)%10)%10;return s+dv}
function validCode(code){const c=String(code||'').replace(/\D/g,'');if(c.length!==20)return false;const s=c.slice(0,19),dv=+c[19];const calc=[...s].reduce((a,ch,i)=>a+(+ch)*(i%2?1:2),0);return ((10-calc%10)%10)===dv}
...
const POLITICOS=[
{id:'maria',nome:'Maria Silva',cargo:'Deputada Federal (SP)',partido:'PV',icm:82,n:1240,variacao:+2,selo:true,comp:{resposta:95,cumprimento:80,devolucao:6},
...
extrato:[{d:'2026-03-12',t:'Depósito',desc:'2.140 votos de confiança ativos',icon:'fa-arrow-down'},{d:'2026-05-02',t:'Promessa',desc:'Creche registrada com lastro',icon:'fa-file-signature'},{d:'2026-06-10',t:'Cobrança',desc:'4.312 eleitores cobraram',icon:'fa-bullhorn'},{d:'2026-07-01',t:'Resposta',desc:'Respondeu em 2 dias',icon:'fa-reply'},{d:'2026-08-01',t:'Cumprimento',desc:'Obra no plano executivo',icon:'fa-circle-check'}],
contracheque:{salario:41500,gabinete:98200,custoDia:456,nota:'Valores ilustrativos'},agenda:{pl1234:'aprova',pl2026:'rejeita',pl45:'aprova',pl2100:'rejeita',pl88:'aprova',pl1500:'rejeita'},resposta:{prazo:'2 dias',direito:'Não acionado.'}},
{id:'carlos',nome:'Carlos Rocha',cargo:'Deputado Federal (MG)',partido:'PSD',icm:54,n:980,variacao:-3,selo:true,comp:{resposta:50,cumprimento:55,devolucao:30},
...
extrato:[{d:'2026-03-12',t:'Depósito',desc:'980 votos ativos',icon:'fa-arrow-down'},{d:'2026-06-02',t:'Cobrança',desc:'1.204 cobraram',icon:'fa-bullhorn'},{d:'2026-07-15',t:'Silêncio',desc:'Sem resposta há 60 dias',icon:'fa-hourglass-half'},{d:'2026-08-02',t:'Estorno',desc:'294 revogaram',icon:'fa-rotate-left'}],
contracheque:{salario:41500,gabinete:98200,custoDia:456,nota:'Valores ilustrativos'},agenda:{pl1234:'rejeita',pl2026:'rejeita',pl45:'aprova',pl2100:'aprova',pl88:'rejeita',pl1500:'aprova'},resposta:{prazo:'—',direito:'Usou direito de resposta em 02/05/2026.'}},
{id:'joao',nome:'João Lima',cargo:'Senador (RJ)',partido:'PL',icm:31,n:1510,variacao:-8,selo:false,comp:{resposta:15,cumprimento:22,devolucao:51},
...
extrato:[{d:'2026-03-12',t:'Depósito',desc:'1.510 votos ativos',icon:'fa-arrow-down'},{d:'2026-05-20',t:'Cobrança',desc:'8.940 cobraram',icon:'fa-bullhorn'},{d:'2026-06-30',t:'Quebra',desc:'Votou contra a própria promessa',icon:'fa-triangle-exclamation'},{d:'2026-07-10',t:'Estorno',desc:'771 revogaram',icon:'fa-rotate-left'}],
contracheque:{salario:41000,gabinete:105000,custoDia:449,nota:'Valores ilustrativos'},agenda:{pl1234:'rejeita',pl2026:'aprova',pl45:'rejeita',pl2100:'rejeita',pl88:'aprova',pl1500:'rejeita'},resposta:{prazo:'—',direito:'Não respondeu.'}}];
const PLS_FALLBACK=[
{id:'pl1234',num:'PL 1234/2026',tema:'Transparência',resumo:'Todo gabinete deve publicar online onde vai cada real da verba pública.',oficial:'Exige publicação mensal detalhada dos gastos de gabinete.'},
{id:'pl2026',num:'PL 2026/2026',tema:'Saúde',resumo:'Previne antes de remediar: mais recursos para UBS e exames.',oficial:'Amplia repasse federal para atenção primária.'},
{id:'pl45',num:'PL 45/2026',tema:'Segurança pública',resumo:'Transparência total das ações e verbas de segurança.',oficial:'Cria painel público unificado de ações e gastos.'},
{id:'pl2100',num:'PL 2100/2026',tema:'Educação',resumo:'Escola em tempo integral em regiões vulneráveis.',oficial:'Programa nacional de ampliação da jornada.'},
{id:'pl88',num:'PL 88/2026',tema:'Tecnologia',resumo:'Governo aberto por padrão: dados legíveis por máquina.',oficial:'Política de dados abertos em todos os órgãos.'},
{id:'pl1500',num:'PL 1500/2026',tema:'Cidadania',resumo:'Facilita acompanhamento de PLs por qualquer cidadão.',oficial:'Simplifica o portal de participação.'}];
const Trilha={
...
acertar(id){const s=this.estado();if(s.feitas[id])return s;s.feitas[id]=new Date().toISOString();LS.set('mb_aulas',s.feitas);LS.set('mb_xp',LS.get('mb_xp',0)+((window.MB_CONFIG&&MB_CONFIG.trilha&&MB_CONFIG.trilha.xpPorAula)||10));return this.registrar()}};
